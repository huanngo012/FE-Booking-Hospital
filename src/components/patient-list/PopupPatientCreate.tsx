...
const PopupPatientCreate = () => {
  ...
  return (
    <>
      ...
      >
        <Typography variant={isTablet ? 'button2' : 'button3'}>Thêm hồ sơ</Typography>
      </Button>
      <Dialog
        open={open}
        onClose={handleClose}
                  ...
                  <Stack direction={direction} gap='16px' width='100%' key={index}>
                    {el?.map((el2: any, index: any) => (
                      <Stack gap='8px' width='100%' key={index}>
                        ...
                        {el2?.select && (
                          <CustomSelect options={el2?.options} placeholder={t(`${el2?.placeholder}`)} value={payload[`${el2?.nameKey}`] || ''} setValue={setPayload} nameKey={el2?.nameKey} />
                        )}
                        {el2?.input && (
                          <TextField
                            type={el2?.type}
                        ...
                        {el2?.date && (
                          <DatePicker
                            defaultValue={moment(payload[`${el2?.nameKey}`] || '')}
                            sx={{
