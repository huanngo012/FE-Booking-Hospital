...
const PopupEditComment = ({ data }: { data?: any }) => {
  ...
  return (
    <>
      <Button variant='outlined' color='tertiary' onClick={handleClickOpen}>
        <Typography variant={isTablet ? 'body2' : 'body3'}>Chỉnh sửa</Typography>
      </Button>
      <Dialog
        open={open}
        onClose={handleClose}
                ...
                <Stack flexDirection='column' gap='16px'>
                  ...
                  <Stack flexDirection='row' justifyContent='center' alignItems='center' gap='16px'>
                    {voteOptions.map((el) => (
                      ...
                      >
                        {Number(payload?.star) && payload?.star >= el.id ? <AiFillStar color='orange' /> : <AiOutlineStar color='var(--grey-500)' />}
                      </Stack>
                    ))}
                  </Stack>
                </Stack>
