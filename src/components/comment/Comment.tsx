    ...
    >
      ...
      <Stack flexDirection={isTablet ? 'row' : 'column'} alignItems='center' gap='16px' width='100%'>
        <Stack flexDirection='column' alignItems='center' justifyContent='center' width={isTablet ? '40%' : '100%'}>
          ...
          <Typography>
            {renderStartFromNumber(totalRatings, 24)?.map((el, index) => <span key={index}>{el}</span>)}
          </Typography>
          ...
          {Array.from(Array(5).keys())
            ...
            .map((el) => (
              <Votebar
                ...
                ratingCount={ratings?.filter((i: any) => i.star === el + 1)?.length}
              />
            ))}
        </Stack>
      </Stack>
      {/* {ratings?.some((el: any) => el?.postedBy?._id === current?._id) ? (
        <Stack flexDirection="column" alignItems="center">
          <Typography variant="label1">
            {t("healthcare facilities.have rv")}
          </Typography>
        </Stack>
      ) : (
        <Stack flexDirection="column" alignItems="center">
          <Typography variant="label1">
            {t("healthcare facilities.rv of hospital")}
          </Typography>
          {popUpComment}
        </Stack>
      )} */}
